...
LocalWhisper e una piccola app macOS locale stile WhisperFlow: tieni premuto Control per registrare, rilasci Control per trascrivere offline con Whisper Large V3 Turbo via `whisper.cpp` (Metal + Accelerate), poi il testo viene copiato negli appunti e, se abilitato, incollato nell'app attiva. Il sorgente vive sotto iCloud Drive in `~/Library/Mobile Documents/com~apple~CloudDocs/Progetti/LocalWhisper`. Il modello Turbo invece NON sta in iCloud: vive in `~/Library/Application Support/LocalWhisper/Models/`.
...
- **Preparare whisper.cpp e Large V3 Turbo**: `./script/setup_whisper_cpp.sh`
...
- Il modello Large V3 Turbo NON deve stare in iCloud Drive: il lazy-load di iCloud rende il primo mmap del modello molto lento. In Application Support cold start e piu prevedibile.
